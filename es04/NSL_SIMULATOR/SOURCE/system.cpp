  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutpv << "#DISTANCE:      BLOCK_NUMBER:        ACTUAL_POFV:         AVE_POFV:        ERROR:" << endl;
        coutpv.close(); 
        input>>_n_bins_v;
        _nprop += _n_bins_v;
...
void System :: finalize(){
  ...
  if (_measure_pofv){
    ...
    double average[_n_bins_v], average2[_n_bins_v], appo; 
    ...
    if (pofvdat.is_open()){
      ...
      pofvdat >> string; pofvdat >> string; pofvdat >> string; pofvdat >> string; pofvdat >> string;
...
void System :: write_pofv(int nconf){
  if (_measure_pofv == true){
  ofstream coutf; 
  coutf.open("../OUTPUT/POFV/pofv" + to_string(nconf) + ".dat"); 
  if (coutf.is_open()){
    coutf << "# VELOCITY:     POFV:" << endl; 
    for (int i=0; i<_n_bins_v; i++){
      coutf << i*_bin_size_v << " " << setw(16) << _measurement(_index_pofv + i) << endl; 
    }
  }
  else{
    cerr << "problem opening pofv" + to_string(nconf) + ".dat"; 
    exit(EXIT_FAILURE); 
  }
  }
  else{
    cerr << "PROBLEM: pofv is not being mesured"; 
    exit(EXIT_FAILURE); 
  } 
  return; 
}
...
double System :: getparticlevelocity(int i, int j){
  return _particle(i).getvelocity(j); 
} 
...
void System :: invert_velocities(){
  ...
  //invert the direction of the velocities 
  for (int i=0; i<_npart; i++){
    for (int j=0; j<_ndim; j++){
      _particle(i).setvelocity(j, -_particle(i).getvelocity(j)); 
    }
  }

   //initialize the _xold coordinates, according to the new velocity 
    double xold, yold, zold; 
    for (int i=0; i<_npart; i++){
      xold = this->pbc( _particle(i).getposition(0,true) - _particle(i).getvelocity(0)*_delta, 0);
      yold = this->pbc( _particle(i).getposition(1,true) - _particle(i).getvelocity(1)*_delta, 1);
