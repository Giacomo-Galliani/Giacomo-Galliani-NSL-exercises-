  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        if (_sim_type ==0){
        ofstream coutp("../OUTPUT/potential_energy_MD.dat");
        coutp << "#     BLOCK:  ACTUAL_PE:     PE_AVE:      ERROR:" << endl;
        coutp.close();
        }
        if (_sim_type ==1){
        ofstream coutp("../OUTPUT/potential_energy_MC.dat");
        coutp << "#     BLOCK:  ACTUAL_PE:     PE_AVE:      ERROR:" << endl;
        coutp.close();
        }
        if (_sim_type >1){
        ...
        index_property++;
        _vtail = 8*M_PI*_rho*(1./(9*pow(_r_cut,9))- 1./(3*pow(_r_cut,3))); // internal energy tail correction 
        ...
        if (_sim_type ==0){
        ofstream coutpr("../OUTPUT/pressure_MD.dat");
        coutpr << "#     BLOCK:   ACTUAL_P:     P_AVE:       ERROR:" << endl;
        coutpr.close();
        }
        if (_sim_type ==1){
        ofstream coutpr("../OUTPUT/pressure_MC.dat");
        coutpr << "#     BLOCK:   ACTUAL_P:     P_AVE:       ERROR:" << endl;
        coutpr.close();
        }
        if (_sim_type >1){
        ...
        if(_sim_type ==0){
        ofstream coutgr("../OUTPUT/gofr_MD.dat");
        ...
        if(_sim_type ==1){
        ofstream coutgr("../OUTPUT/gofr_MC.dat");
        coutgr << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
        coutgr.close();
        coutgr.open("../OUTPUT/gofrblocks.dat"); 
        coutgr << "#DISTANCE:      BLOCK_NUMBER:        AVE_GOFR:" << endl;
        ...
        coutpv.close();
        coutpv.open("../OUTPUT/pofvblocks.dat"); 
        coutpv << "#DISTANCE:      BLOCK_NUMBER:        AVE_POFV:" << endl;
        coutpv.close(); 
        input>>_n_bins_v;
        _nprop += _n_bins_v;
...
void System :: finalize(){
  ...
  if (_measure_gofr){
    ...
    if (gofrdat.is_open()){
      ...
      //fill accumulators with data from file 
    ...
    for (int i=0; i<_n_bins; i++){
      average[i] /= double(_nblocks); 
      average2[i] /= double(_nblocks);  
      average2[i] -= average[i]*average[i]; 
    }

    if (_sim_type ==0){
      ofstream gofrout; 
      gofrout.open("../OUTPUT/gofr_MD.dat", ios::app); 

      for (int i=0; i<_n_bins; i++){
        gofrout << setw(12) << i*_bin_size
                << setw(12) << average[i]
                << setw(12) << sqrt(average2[i]/double(_nblocks)) << endl; 
      }
    }

    if (_sim_type ==1){
      ofstream gofrout; 
      gofrout.open("../OUTPUT/gofr_MC.dat", ios::app); 
  ...
  if (_measure_pofv){
    ...
    string string; 
    ifstream pofvdat; 
    pofvdat.open("../OUTPUT/pofvblocks.dat"); 
    if (pofvdat.is_open()){

      pofvdat >> string; pofvdat >> string; pofvdat >> string;
      ...
      cerr << "error opening pofvblocks.dat"; 
      exit(EXIT_FAILURE); 
    }
    ...
    for (int i=0; i<_n_bins; i++){
      ...
      ofstream pofvout; 
      pofvout.open("../OUTPUT/pofv.dat", ios::app); 

      for (int i=0; i<_n_bins; i++){
        pofvout << setw(12) << i*_bin_size_v
                << setw(12) << average[i]
                << setw(12) << sqrt(average2[i]/double(_nblocks)) << endl; 
      }
      pofvout.close(); 
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        // GOFR
        if(dr < _r_cut){
          if(_measure_gofr) _measurement(_index_gofr + static_cast<int>(dr*_n_bins/_r_cut))+=2./(_rho*_npart*(4./3.)*M_PI*(pow(static_cast<int>(dr*_n_bins/_r_cut)*_bin_size + _bin_size,3)- pow(static_cast<int>(dr*_n_bins/_r_cut)*_bin_size, 3))); //GOFR fill normalized histogram        
  ...
  if(_measure_pofv){
    ...
    for (int i=0; i<_n_bins_v; i++){
      _measurement(_index_pofv +i) = 0; }

    for (int i=0; i<_npart; i++){

        //compute modulus of the velocity for each particle 
        for (int l=0; l<_ndim; l++){
            v += pow(_particle(i).getvelocity(l), 2); 
        }
        v = sqrt(v); 

        //check if the velocity isn't too high 
        if (static_cast <int> (v/_bin_size_v) < _n_bins_v)
          //fill the histogram 
          _measurement(_index_pofv + static_cast<int>(v/_bin_size_v))++; 
        v=0; 
      }
...
void System :: averages(int blk){
  ...
  if (_measure_penergy){
    if (_sim_type ==0){
      coutf.open("../OUTPUT/potential_energy_MD.dat",ios::app);
    ...
    if (_sim_type ==1){
      coutf.open("../OUTPUT/potential_energy_MC.dat",ios::app);
      average  = _average(_index_penergy);
      sum_average = _global_av(_index_penergy);
      sum_ave2 = _global_av2(_index_penergy);
      coutf << setw(12) << blk 
  ...
  if (_measure_pressure){
    if (_sim_type ==0){
      coutf.open("../OUTPUT/pressure_MD.dat",ios::app);
    ...
    if (_sim_type ==1){
      coutf.open("../OUTPUT/pressure_MC.dat",ios::app);
      average  = _average(_index_pressure);
      sum_average = _global_av(_index_pressure);
      sum_ave2 = _global_av2(_index_pressure);
      coutf << setw(12) << blk
  ...
  if (_measure_pofv){
    coutf.open("../OUTPUT/pofvblocks.dat",ios::app);
    for (int i=0; i<_n_bins_v; i++){
