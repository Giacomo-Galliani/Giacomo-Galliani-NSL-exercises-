...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double p = _particle(this ->pbc(i+1)).getspin() + _particle(this ->pbc(i-1)).getspin();  

    p = 1/(1+ exp(_beta*(-2*_J*p - 2*_H)));
    if (p>1){
      cerr << "error: invalid evaluation of probability distribution" << endl; 
      exit(EXIT_FAILURE); 
    } 
    if (_rnd.Rannyu() > p)
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8*M_PI*_rho*(1./(9*pow(_r_cut,9))- 1./(3*pow(_r_cut,6))); // TO BE FIXED IN EXERCISE 7
        ...
        index_property++;
        _ptail = 32*M_PI*_rho*(1./(9*pow(_r_cut,9))- 1./(6*pow(_r_cut,6))); // TO BE FIXED IN EXERCISE 7
      } else if( property == "GOFR" ){
        ofstream coutgr("../OUTPUT/gofr.dat");
        coutgr << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
        coutgr.close();
        coutgr.open("../OUTPUT/gofrblocks.dat"); 
        coutgr << "#DISTANCE:      BLOCK_NUMBER:        AVE_GOFR:" << endl;
        coutgr.close(); 
        input>>_n_bins;
        _nprop+=_n_bins;
...
void System :: finalize(){
  ...
  //scrive le medie finali di gofr 
  if (_measure_gofr){
    double average[_n_bins], average2[_n_bins], appo; 
    ...
    string string; 
    ifstream gofrdat; 
    gofrdat.open("../OUTPUT/gofrblocks.dat"); 
    if (gofrdat.is_open()){
      gofrdat >> string; gofrdat >> string; gofrdat >> string;
    ...
    ofstream gofrout; 
    gofrout.open("../OUTPUT/gofr.dat", ios::app); 

    for (int i=0; i<_n_bins; i++){
      gofrout << setw(12) << i*_bin_size
              << setw(12) << average[i]/double(_nblocks)
              << setw(12) << average2[i]/double(_nblocks) - (average[i]/double(_nblocks))*(average[i]/double(_nblocks)) << endl; 
    }

    }
    else {
      cerr << "error opening gofrblocks.dat"; 
      exit(EXIT_FAILURE); 
    }
  }
...
void System :: read_configuration_half_fcc(){
  ifstream cinf;
...
// Read configuration from a .fcc file in directory ../OUTPUT/CONFIG/
void System :: read_configuration_fcc(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config.fcc");
  if(cinf.is_open()){
    string comment;
...
void System :: read_configuration_velocities(){

ifstream cinf;
  cinf.open("../INPUT/CONFIG/configvelocities.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
    ...
    cinf >> ncoord;
    if (ncoord != _npart){
      cerr << "PROBLEM: conflicting number of coordinates in input.dat & configvelocities.xyz not match!" << endl;
      exit(EXIT_FAILURE);
    }
    cinf >> comment;
    for(int i=0; i<_npart; i++){
      cinf >> particle >> x >> y >> z; 
      _particle(i).setvelocity(0, x);
      _particle(i).setvelocity(1, y);
      _particle(i).setvelocity(2, z);
      _particle(i).acceptmove(); // _x_old = _x_new
    }
    cinf.close(); 

    //per il verlet inizializzo _xold
    double xold, yold, zold; 
    for (int i=0; i<_npart; i++){
      xold = this->pbc( _particle(i).getposition(0,true) - _particle(i).getvelocity(0)*_delta, 0);
      yold = this->pbc( _particle(i).getposition(1,true) - _particle(i).getvelocity(1)*_delta, 1);
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _r_cut){
          if(_measure_gofr) _measurement(_index_gofr + static_cast<int>(dr*_n_bins/_r_cut))+=2./(_rho*_npart*(4./3.)*M_PI*(pow(static_cast<int>(dr*_n_bins/_r_cut)*_bin_size + _bin_size,3)- pow(static_cast<int>(dr*_n_bins/_r_cut)*_bin_size, 3)));          
  ...
  if(_measure_pofv){
    ...
    for (int i=0; i<_npart; i++){
        ...
        //determino la velocità (in modulo) di ogni particella 
        for (int l=0; l<_ndim; l++){
            v += pow(_particle(i).getvelocity(l), 2); 
        }
        v = sqrt(v); 

        //controllo se la particella non ha una velocità troppo alta
        if (static_cast <int> (v/_bin_size_v) < _n_bins_v)
          //riempio l'istogramma in modo tale che sia correttamente normalizzato 
          histo(static_cast <int> (v/_bin_size_v)) += 1/double(_npart); 

      }

      ofstream fileout; 
      fileout.open("velocities.txt"); 
      for (int i=0; i<_n_bins_v; i++){
        fileout << i +1 << " " << histo(i) << endl; 
      }
...
// TO BE FIXED IN EXERCISE 6
  if (_measure_magnet){
    ...
    spin_temp /= double(_npart); 
    _measurement(_index_magnet) = spin_temp; 
  }
...
// TO BE FIXED IN EXERCISE 6
  if (_measure_cv){
    ...
    for (int i=0; i<_npart; i++){ //loop over particles 
      s_i = double(_particle(i).getspin()); 
      s_j = double(_particle(this->pbc(i+1)).getspin());
      tenergy_temp += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j); //compute internal energy 
      tenergy2_temp += (-_J * s_i * s_j - 0.5 * _H * (s_i + s_j))*(-_J * s_i * s_j - 0.5 * _H * (s_i + s_j)); //compute mean of H^2
    }
    ...
    tenergy2_temp -= tenergy_temp*tenergy_temp;  //compute variance of the internal energy 
    _measurement(_index_cv) = _beta*_beta*tenergy2_temp; 
  }
  ...
  if(_measure_chi){
    ...
    spin_temp = 0.0; 
    for (int i=0; i<_npart; i++){
      spin_temp += double(_particle(i).getspin()); 
    } 
       
    spin_temp *= spin_temp;
    spin_temp /= double(_npart);

    _measurement(_index_chi) = _beta*spin_temp; 
  }
...
void System :: averages(int blk){
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr){
    coutf.open("../OUTPUT/gofrblocks.dat",ios::app);
    for (int i=0; i<_n_bins; i++){
    ...
    if(_sim_type > 0){
      ofstream ratio; 
      ratio.open("Metropolis_acceptance_rate.dat"); 
      ratio << "METROPOLIS ACCEPTANCE RATE" << endl;
      ratio << double(_naccepted)/double(_nattempts) << "with a uniform distribution in " << -_delta << " " << _delta; 
  ...
  /*if(_measure_pofv){
    coutf.open("../OUTPUT/velocities.dat", ios::app);
...
double System :: get_halfside(int i){
  if (i> _ndim)
    exit(33); 
  else
  return _halfside(i); 
...
void System :: save_T_results(int l){
  ...
  if (l==0){
    if (_measure_tenergy){ 
      results.open("../OUTPUT/tenergy_T_dependence.dat"); 
      results << "# Temperature: " << setw(12) << "Total Energy: " << setw(12) << "Error: " << endl; 
      results.close(); 
    }
    if (_measure_cv) {
      results.open("../OUTPUT/cv_T_dependence.dat");
      results << "# Temperature: " << setw(12) << "Heat Capacity: " << setw(12) << "Error: " << endl; 
      results.close();
    }
    if (_measure_chi){
      results.open("../OUTPUT/chi_T_dependence.dat"); 
      results << "# Temperature: " << setw(12) << "Susceptibility: " << setw(12) << "Error: " << endl; 
      results.close();
      }
    if (_measure_magnet) {
      results.open("../OUTPUT/magnet_T_dependence.dat");
      results << "# Temperature: " << setw(12) << "Magnetization: " << setw(12) << "Error: " << endl; 
      results.close();
      }
  }
  ...
  if (_measure_tenergy){
    dat.open("../OUTPUT/total_energy.dat"); 
    results.open("../OUTPUT/tenergy_T_dependence.dat", ios:: app); 
    if (dat.is_open()){

      dat >> str; dat >> str; dat >> str; dat >> str; dat >> str; 

      do{
        dat >> appo; 
        count++; 
      } while (count < 4*(l+1)*_nblocks -4);
      count =0; 
      ...
      results << _temp << setw(12) << appo << setw(12); 
      ...
      cerr << "problem opening total_energy.dat" << endl; 
      exit(EXIT_FAILURE); 
    }
  }

  if (_measure_cv){
    dat.open("../OUTPUT/specific_heat.dat"); 
    results.open("../OUTPUT/cv_T_dependence.dat", ios ::app); 
    if (dat.is_open()){
      ...
      cerr << "problem opening specific.heat.dat" << endl; 
      exit(EXIT_FAILURE); 
    }
  }

  if (_measure_chi){
    dat.open("../OUTPUT/susceptibility.dat"); 
    results.open("../OUTPUT/chi_T_dependence.dat", ios:: app); 
    if (dat.is_open()){
      ...
      cerr << "problem opening susceptibility.dat" << endl; 
      exit(EXIT_FAILURE); 
    }
  }

  if (_measure_magnet){
    dat.open("../OUTPUT/magnetization.dat"); 
    results.open("../OUTPUT/magnet_T_dependence.dat", ios :: app); 
    if (dat.is_open()){
      ...
      cerr << "problem opening magnetization.dat" << endl; 
      exit(EXIT_FAILURE); 
    }
  }
  ...
  if (_sim_type == 3){
    ...
    if (l==0){
    if (_measure_tenergy){ 
      results.open("../OUTPUT/tenergy_T_dependence_Gibbs.dat"); 
      results << "# Temperature: " << setw(12) << "Total Energy: " << setw(12) << "Error: " << endl; 
      results.close(); 
    }
    if (_measure_cv) {
      results.open("../OUTPUT/cv_T_dependence_Gibbs.dat");
      results << "# Temperature: " << setw(12) << "Heat Capacity: " << setw(12) << "Error: " << endl; 
      results.close();
    }
    if (_measure_chi){
      results.open("../OUTPUT/chi_T_dependence_Gibbs.dat"); 
      results << "# Temperature: " << setw(12) << "Susceptibility: " << setw(12) << "Error: " << endl; 
      results.close();
      }
    if (_measure_magnet) {
      results.open("../OUTPUT/magnet_T_dependence_Gibbs.dat");
      results << "# Temperature: " << setw(12) << "Magnetization: " << setw(12) << "Error: " << endl; 
      results.close();
      }
  }
  ...
  if (_measure_tenergy){
    dat.open("../OUTPUT/total_energy.dat"); 
    results.open("../OUTPUT/tenergy_T_dependence_Gibbs.dat", ios:: app); 
    if (dat.is_open()){
  ...
  if (_measure_cv){
    dat.open("../OUTPUT/specific_heat.dat"); 
    results.open("../OUTPUT/cv_T_dependence_Gibbs.dat", ios ::app); 
    if (dat.is_open()){
  ...
  if (_measure_chi){
    dat.open("../OUTPUT/susceptibility.dat"); 
    results.open("../OUTPUT/chi_T_dependence_Gibbs.dat", ios:: app); 
    if (dat.is_open()){
  ...
  if (_measure_magnet){
    dat.open("../OUTPUT/magnetization.dat"); 
    results.open("../OUTPUT/magnet_T_dependence_Gibbs.dat", ios :: app); 
    if (dat.is_open()){
