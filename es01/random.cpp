...
double Random :: Exp(double lambda) {
   // This function generates a random number from an expontential distribution with given parameter lambda (with cumulative method)
...
double Random :: Lorentz(double mu, double gamma) {
   // This function generates a random number from an lorentian distribution with given parameter gamma (with cumulative method)
   // for x from -infinity to + infinity 

   return (gamma*tan(M_PI*(Rannyu()-1/2))- mu); 
}
